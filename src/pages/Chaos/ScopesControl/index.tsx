...
const ScopesControl: FC = () => {
  ...
  function renderExtra() {
    return <div className={styles.extra} style={{ display: 'none' }}>
      <div>
        {
          Number(type) === SCOPE_TYPE.HOST ?
            <div>
              <Translation>Number of online customers/total number of ECS</Translation>:{totalInfo[type] || '0/0'}
            </div> :
            <div>
  ...
  /** table 空数据展示信息 */
  const emptyTable = <div><Translation>No Data</Translation></div>;
  /** 更新 统计信息 */
  ...
  return (
    <div className={styles.warp}>
      ...
      >
        <Tab.Item title={i18n.t('Abnomal Up').toString()} key={SCOPE_TYPE.HOST}><HostTable {...tableProps} /></Tab.Item>
        <Tab.Item title={i18n.t('Abnomal Down').toString()} key={SCOPE_TYPE.HOST2}><HostTable {...tableProps} /></Tab.Item>
      </Tab>
