...
const Layout: FC = ({ children }: PropsWithChildren<any>) => {
  ...
  const handeSetMenu = (caseType: string) => {
    if (caseType) {
      ...
      setMenuHeader(i18n.t('Apply high availability services').toString());
    }
  };
  ...
  const renderMuenHeader = (key: string) => {
    return (
      <div style={{ width: 110, cursor: 'pointer' }} onClick={() => {
        ...
        <span style={{ marginLeft: 20, position: 'relative', top: 3, fontSize: 16 }}><Translation>Return</Translation></span>
      </div>
    );
  };
  // chaos的菜单鉴权
